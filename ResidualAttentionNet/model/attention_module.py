...
class AttentionModule_stage1(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, size1=(56, 56), size2=(28, 28), size3=(14, 14)):
        ...
        self.softmax3_blocks = nn.SequentialCell([
            ResidualBlock(in_channels, out_channels),
            ResidualBlock(in_channels, out_channels)])
        self.interpolation = ops.ResizeBilinearV2(align_corners=True)
        self.softmax4_blocks = ResidualBlock(in_channels, out_channels)
        self.softmax5_blocks = ResidualBlock(in_channels, out_channels)
        self.softmax6_blocks = nn.SequentialCell([
            nn.BatchNorm2d(num_features=out_channels, momentum=0.9),
    ...
    def construct(self, x):
        ...
        out_softmax3 = self.softmax3_blocks(out_mpool3)
        out_interp3 = self.interpolation(out_softmax3, self.size3) + out_softmax2
        out = out_interp3 + out_skip2_connection
        out_softmax4 = self.softmax4_blocks(out)
        out_interp2 = self.interpolation(out_softmax4, self.size2) + out_softmax1
        out = out_interp2 + out_skip1_connection
        out_softmax5 = self.softmax5_blocks(out)
        out_interp1 = self.interpolation(out_softmax5, self.size1) + out_trunk
        out_softmax6 = self.softmax6_blocks(out_interp1)
        out = (1 + out_softmax6) * out_trunk
        out_last = self.last_blocks(out)
        return out_last
...
class AttentionModule_stage2(nn.Cell):
    ...
    def construct(self, x):
        ...
        out_mpool2 = self.mpool2(opt_pad2)
        out_softmax2 = self.softmax2_blocks(out_mpool2)
        out_interp2 = self.interpolation(out_softmax2, self.size2) + out_softmax1
        out = out_interp2 + out_skip1_connection
        out_softmax3 = self.softmax3_blocks(out)
        out_interp1 = self.interpolation(out_softmax3, self.size1) + out_trunk
...
class AttentionModule_stage3(nn.Cell):
    ...
    def construct(self, x):
        ...
        out_softmax1 = self.softmax1_blocks(out_mpool1)
        out_interp1 = self.interpolation(out_softmax1, self.size1) + out_trunk
...
class AttentionModule_stage1_cifar(nn.Cell):
    ...
    def construct(self, x):
        ...
        out_interp = self.interpolation(out_middle_2r_blocks, self.size2) + out_down_residual_blocks1
        out = out_interp + out_skip1_connection
        out_up_residual_blocks1 = self.up_residual_blocks1(out)
        out_interp2 = self.interpolation(out_up_residual_blocks1, self.size1) + out_trunk
...
class AttentionModule_stage2_cifar(nn.Cell):
    ...
    def construct(self, x):
        ...
        out_interp = self.interpolation(out_middle_2r_blocks, self.size) + out_trunk
