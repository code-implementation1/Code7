...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, scale=4):
    ...
    if factor is not None:
        sfx = '_{}'.format(factor)
        _minify(basedir, factors=[factor/scale])
        factor = factor
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, path_zflat=False, scale=4):
    poses, bds, imgs = _load_data(basedir, factor=factor, scale=scale) # factor=8 downsamples original imgs by 8x
